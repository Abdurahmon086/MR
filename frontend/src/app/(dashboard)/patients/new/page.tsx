"use client";
import { Card, Form, Input, Button, Select, DatePicker, Row, Col, Typography, message } from "antd";
...
export default function NewPatientPage() {
  ...
  const mutation = useMutation({
    mutationFn: (values: any) => patientsApi.create({ ...values, birth_date: values.birth_date?.format("YYYY-MM-DD") }),
    onSuccess: () => {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <Title level={3} style={{ margin: 0 }}>{t("patients.add")}</Title>
      </div>
          ...
          <Row gutter={16}>
            ...
            <Col xs={24} md={8}>
              <Form.Item name="blood_type" label="Qon guruhi">
                <Select allowClear>
                  {["A+","A-","B+","B-","AB+","AB-","O+","O-"].map((b) => <Select.Option key={b} value={b}>{b}</Select.Option>)}
                </Select>
              </Form.Item>
            </Col>
            <Col xs={24} md={8}>
            ...
            <Col xs={24} md={8}>
              <Form.Item name="assigned_doctor_id" label={t("patients.assignedDoctor")}>
                <Select allowClear showSearch optionFilterProp="children" placeholder="Shifokorni tanlang">
                  {doctors?.map((d: any) => <Select.Option key={d.id} value={d.id}>{d.full_name}</Select.Option>)}
                </Select>
              </Form.Item>
            </Col>
            <Col xs={24}>
